...
namespace msgpack
{
  ...
  public:
    ...
    void pack_array_header(std::size_t size)
    {
      if (size < 16)
      {
        ...
        serialized_object.emplace_back(uint8_t(size | size_mask));
      }
      else if (size < std::numeric_limits<uint16_t>::max())
      {
        ...
        {
          serialized_object.emplace_back(uint8_t(size >> (8U * (i - 1)) & 0xff));
        }
      }
  ...
  public:
    ...
    std::size_t unpack_array_header()
    {
      ...
      else if (safe_data() >= fixarray_start && safe_data() <= fixarray_end)
      {
    ...
    bool is_next_unsigned()
    {
      uint8_t next = safe_data();
      return next == uint8 || next == uint16 || next == uint32 || next == uint64;
    }
